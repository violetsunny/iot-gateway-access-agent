 ...
 */
package cn.enncloud.iot.gateway.mqtt.client;
...
final class MqttClientImpl implements MqttClient {
    ...
    private final Map<String, MqttHandler> subscriptionBackups = new ConcurrentHashMap<>();
...
//            }
            eventLoop.schedule((Runnable) () -> {
                ...
                connect(host, port, reconnect)
                        .addListener(cf -> {
                            ...
                            if (result.isSuccess()) {
                                //connected.set(true); connect已经变更
                                //retry.set(false);
    ...
    public Future<Void> on(String topic, MqttHandler handler, MqttQoS qos) {
        subscriptionBackups.put(topic, handler);//增加订阅列表的备份
    ...
    public Future<Void> off(String topic, MqttHandler handler) {
        ...
        this.subscriptionBackups.remove(topic);
